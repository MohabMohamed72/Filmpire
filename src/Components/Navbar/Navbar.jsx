...
let y = 18;
  ...
  return (
    <div className={`${(dark)? `bg-[#000000d5] `:`bg-[#1976d2] shadow-xl`}   shadow-xl shadow-slate-400 max-h-[80px] sticky w-full  transition-all duration-700 z-[1000] `}>
      <div className=' w-full h-full relative flex justify-between items-center px-5 py-5 max-sm:px-5'>
          ...
          <div className={`${SearchVisable? `block`:`hidden`} relative  ml-auto mr-auto `}>
            ...
            <input type="text"  className=' transition-all duration-700 border-b-2 focus:border-orange-400 text-white text-[12px] px-10 max-sm:px-0 py-1  bg-transparent focus:outline-none' onChange={(e)=>{
          ...
          <div className='flex items-center gap-3 cursor-pointer' onClick={()=>{
            ...
            <IoMdPerson className='size-5 text-white cursor-pointer '/>
          </div> 
      ...
      <div className={`${(SidebarVisabilty)? `block` : `hidden` } transition-all duration-700`}>
        <div className='absolute top-0 left-0 transition-all duration-700 ' onClick={()=>{
          ...
          <IoIosCloseCircleOutline className='absolute top-2 left-2 cursor-pointer text-blue-600 size-5'onClick={()=>{
            setSidebarVisabilty(false)
          }} />
          <Sidebar />
        </div>
